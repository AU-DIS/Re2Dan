"""
Class for models for semantic search using make_multilingual script
"""
...
class ModelTranslated(SearchEngine):
    def __init__(self, model_path, text_path, embedding_path):
    ...
    def get_results(self, query, top_k=5, unit=None, department=None):
        ...
        data, embeddings = super().filter_docs(self.data, self.embeddings, unit, department)

        hits = util.semantic_search(query_embedding, embeddings, top_k=top_k)[0]
        for i, h in enumerate(hits):
            rank = i+1
            similarity_score = h['score']
            title = data.iloc[h['corpus_id']]['title']
            docid = data.iloc[h['corpus_id']]['docid']
            url = data.iloc[h['corpus_id']]['url']
...
if __name__ == "__main__":
    ...
    embedding_path = f"re2dan/files/embeddings/edok_corpus_embeddings_{model_name}.pkl"
    text_path = "re2dan/files/documents/3_extracting/edok_text_extracted.pkl"
    model = ModelTranslated(model_path, text_path, embedding_path)
