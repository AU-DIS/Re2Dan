...
********************************************************************************************
Code modified from 
https://github.com/UKPLab/sentence-transformers/tree/master/examples/training/multilingual
********************************************************************************************
...
# Teacher model based on https://www.sbert.net/examples/applications/semantic-search/README.html
teacher_model_name = 'sentence-transformers/multi-qa-mpnet-base-cos-v1'   #Our monolingual teacher model, we want to convert to multiple languages
student_model_name = 'sentence-transformers/paraphrase-multilingual-mpnet-base-v2'       #Multilingual base model we use to imitate the teacher model

max_seq_length = 128                #Student model max. lengths for inputs (number of word pieces)
train_batch_size = 64               #Batch size for training
inference_batch_size = 64           #Batch size at inference
max_sentences_per_language = 2000000 # 500000 #Maximum number of  parallel sentences for training
train_max_sentence_length = 500     #Maximum length (characters) for parallel training sentences

num_epochs = 5                       #Train for x epochs
num_warmup_steps = 1000 # 10000             #Warumup steps
...
target_languages = set(['da'])    # We want to extend the model to these new languages. For language codes, see the header of the train file
...
# Preprocessed files
train_files = ['parallel-sentences/all_parallel_data_train.tsv.gz']
dev_files = ['parallel-sentences/all_parallel_data_dev.tsv.gz']
...
for dev_file in dev_files:
    ...
    with gzip.open(dev_file, 'rt', encoding='utf8') as fIn:
        for line in fIn:
            ...
            if len(splits)==2 and splits[0] != "" and splits[1] != "":
